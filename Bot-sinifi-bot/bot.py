...
async def birazgülolum(ctx):
    with open(r'Bot-sinifi-bot\smile.jpg', 'rb') as f:
...
async def bayrak_göster_asker(ctx):
    with open(r'Bot-sinifi-bot\flag.jpg', 'rb') as f:
...
async def izmirmarş(ctx):
    with open(r'Bot-sinifi-bot\Haluk Levent - İzmir Marşı.mp3', 'rb') as f:
        sarki = discord.File(f)
    await ctx.send(file=sarki)
@bot.command()
async def çanakkalemarş(ctx):
    with open(r'Bot-sinifi-bot\Çanakkale içinde aynalı çarşı, ana ben gidiyorum düşmana karşı .....mp3', 'rb') as f:
...
async def istiklalmarş(ctx):
    with open(r'Bot-sinifi-bot\İstiklal Marşı.mp3', 'rb') as f:
        mars = discord.File(f)
    await ctx.send(file=mars)
@bot.command()
async def atam(ctx):
    with open(r'Bot-sinifi-bot\Ataturk.jpg', 'rb') as f:
