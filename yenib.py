...
async def kendini_tanıt(ctx):
    intro = (
        ...
        "size eğlenceli komutlar sunmak için buradayım! 😄\n"
        "İşte bazı komutlar:\n"
        "*merhaba - Size selam gönderirim!\n"
        "*şaka - Komik bir şaka paylaşırım!\n"
        "*söz - Bir özlü söz paylaşırım!"
    )
...
@bot.command()
async def selam(ctx):
    await ctx.send(f"Merhaba {ctx.author.name}! Nasılsın? 😊")
@bot.command()
...
@bot.command()
async def durum(ctx):
    await ctx.send(f"Bot şu anda {len(bot.guilds)} sunucuda çalışıyor ve {len(bot.users)} kullanıcıya hizmet veriyor!")
@bot.command()
async def tarih(ctx):
    bilgiler = [
